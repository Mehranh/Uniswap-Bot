...
export function fetchAbiFromEtherscan(contract: any) {
    ...
    return axios.get(URL).then((response: any) => {
        ...
        console.log(`Error while fetching contract ${contract} from etherscan with error: ${err}`);
    });
}
...
export function fixedDecimal(num,fixed:number) {
    var re = new RegExp('^-?\\d+(?:\.\\d{0,' + (fixed || -1) + '})?');
    var with2Decimals = num.toString().match(re)[0]
